...
class FingerprintSystem:
    def __init__(self, root):
        self.root = root; self.root.title("ระบบตรวจสอบลายนิ้วมือ OCT (v5.1 - Fix NameError)"); self.root.geometry("1250x750") # Updated title
    ...
    def setup_verify_tab(self):
        ...
        self.verify_tab.columnconfigure(0,weight=1); self.verify_tab.columnconfigure(1,weight=1); self.verify_tab.rowconfigure(0,weight=1); lf.rowconfigure(1,weight=1); self.results_frame.columnconfigure(1,weight=1); self.results_frame.rowconfigure(4,weight=1)
    ...
    def select_capture_area(self, mode):
        ...
        elif mode=="verify" and self.is_verifying_live: self.stop_live_verify() # Use correct stop function
    ...
    def start_capture_register(self):
        if self.is_capturing_register or self.is_verifying_live: messagebox.showwarning("กำลังทำงาน","แสดงผลสดอยู่แล้ว",parent=self.root); return # Check is_verifying_live too
    ...
    def _capture_frame_action(self, source_canvas, mode):
        ...
        # elif mode=="verify" and self.is_verifying_live: was_capturing=True; self.stop_live_verify() # Don't stop live verify just to capture
        # Let live verify continue, just grab a frame
    ...
    def _draw_placeholder(self, canvas, text):
         try:
             ...
             if width>1 and height>1:
                 is_capturing=(canvas==self.scan_canvas_register and self.is_capturing_register)or(canvas==self.scan_canvas_verify and self.is_verifying_live) # Check is_verifying_live here
    ...
    # verify_fingerprint is no longer needed for a button, but the logic is in _compare_features and _update_verification_ui
    ...
    def display_scan(self, img_np, canvas, max_width=None, max_height=None, is_hog=False):
        ...
        try:
            ...
            cname=canvas.winfo_name() # กำหนดค่า cname
            ...
            nw=max(1,int(iw*scale)); nh=max(1,int(ih*scale)); # nw, nh assigned here
            ...
            if is_hog: canvas.create_text(5,5,anchor="nw",text="HOG",fill="blue",font=("Arial",8),tags="hog_text")
        except Exception as e:
            print(f"ERROR display {cname}: {e}") # cname ควรมีค่าแล้ว
    ...
    def _clear_ui_state(self):
         ...
         self._clear_canvas(self.scan_canvas_register,"1. เลือกพื้นที่จับภาพ"); self._clear_canvas(self.scan_canvas_verify,"1. เลือกพื้นที่จับภาพ"); self._clear_canvas(self.admin_preview_canvas,"ภาพตัวอย่าง"); self._clear_canvas(self.hog_canvas,"HOG Visualization")
